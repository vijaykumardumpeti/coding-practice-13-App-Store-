...
export default class AppStore extends Component {
  ...
  render() {
    ...
    const filteredList = appsList.filter(
      eachObject => eachObject.category === id,
    )
    ...
    const resultList = filteredList.filter(eachObject =>
        ...
        <div className="app-store-container">
          ...
          <ul className="app-items-container">
            {resultList.map(eachObject => (
              <AppItem eachObject={eachObject} key={eachObject.appId} />
            ))}
          </ul>
        </div>
      </div>
